...
WAJIB_JOIN = list( 
    os.environ.get( 
        "WAJIB_JOIN", "EjaKentang" 
    ).split() 
)
USENAME_OWNER = os.environ.get("USENAME_OWNER", "@DoiAnda")
...
BLACKLIST_GCAST = get_blacklist() or []
...
SAWERIA_USERNAME = os.environ.get("SAWERIA_USERNAME", "")
RASTA = list(
    map(
        int,
        os.environ.get(
