...
async def copyall_cmd(client, message):
    ...
    if len(args) < 2:
        return await message.reply(
            f"**Please give link**\nExample: `{message.text.split()[0]} https://t.me/root4o4dev/1745 5`"
        )
    ...
    if chats is None or links is None:
        return await proses.edit(
            f"**Invalid link**\nExample: `{message.text.split()[0]} https://t.me/root404dev/1745 5`"
        )
    try:
...
async def copymsg_cmd(client, message):
    em = Emoji(client)
    await em.get()
    args = message.command[1:]
